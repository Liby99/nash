...
void ThirdPersonCamera::update() {
  ...
  if (context->getKey(' ')) vel.y() += 1;
  if (context->getKey('x')) vel.y() -= 1;
  if (context->getKey('w')) vel -= Vector3f(sin(azimuth), 0, cos(azimuth));
  if (context->getKey('s')) vel += Vector3f(sin(azimuth), 0, cos(azimuth));
  if (context->getKey('a')) vel += Vector3f(-cos(azimuth), 0, sin(azimuth));
  if (context->getKey('d')) vel += Vector3f(cos(azimuth), 0, -sin(azimuth));
