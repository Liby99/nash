...
Camera::Camera()
    : position(0, 0, -1), target(0, 0, 0), up(0, 1, 0), width(1280),
      height(720), zNear(0.1), zFar(20), aspect(1.7777778), fovy(45),
      control(nullptr) {
  // Do nothing
}
...
Matrix4f Camera::getView() {
  ...
  z.normalize();
  Matrix4f mat;
  mat << x[0], x[1], x[2], -x.dot(position), y[0], y[1], y[2], -y.dot(position),
      z[0], z[1], z[2], -z.dot(position), 0, 0, 0, 1;
  return mat;
}
